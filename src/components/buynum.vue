<template>
  ...
  <div class="item-cols-num clearfix">
    <div class="select">
      <span class="down"
            :class="{'down-disabled':Num<=1||isdisable==1}"
      ...
      <span class="up" 
            :class="{'up-disabled':Num>=limit||isdisable==1}"
...
<script>
  export default {
    ...
    methods: {
      up () {
        if (this.flag && this.Num < this.limit&&this.isdisable!=1) {
          this.ani('up')
        }
        return false
      },
      down () {
        if (this.flag && this.Num > 1&&this.isdisable!=1) {
          this.ani('down')
        }
        return false
      },
      blur () {
        ...
        this.$emit('edit-num', this.num, this.id, this.checked , this.goodlistid  , this.iAll ,this.iin)
      },
      ani (opera) {
        this.flag = false
        let n = this.Num
