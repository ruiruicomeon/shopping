...
<script>
import { selectGoodsList } from "/api/goods.js";
...
export default {
  ...
  methods: {
    ...
    _getAllGoods() {
      ...
      const lid =
        isNotBlank(this.$route.query) && isNotBlank(this.$route.query.id)
          ? this.$route.query.id
          : "";
      selectGoodsList({
        isSort: 0,
  ...
  watch: {
    $route(to, from) {
      // if (to.fullPath.indexOf("/goods?id=") >= 0) {
      //   this.cId = to.query.id;
      this._getAllGoods();
      this.loading = true;
      // }
    }
