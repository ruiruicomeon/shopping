        ...
        <div v-if="loginPage" class="content">
          <ul class="common-form">
            ...
            <!-- <li style="text-align: right" class="pr">
              ...
              <a href="javascript:;" class="register" @click="toRegister">注册 XFM 账号</a>
...
import { userLogin1, userInfo1 ,getUserInfo } from "/api/index.js";
import { addCart } from "/api/goods.js";
import { setStore, getStore, removeStore, isNotBlank } from "/utils/storage.js";
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      const a = `${this.ruleForm.userName}:${this.ruleForm.userPwd}`;
      const base64Str = window.btoa(a);
      const loginMsg = { Authorization: `Basic ${base64Str}` };

      userLogin1(loginMsg, { headers: loginMsg }).then(res => {
        if (isNotBlank(res.success) && res.success == 1) {
          sessionStorage.setItem("token", res.data);
          getUserInfo().then(resin=>{
            setStore('mallName',resin.data.name)
            this.RECORD_USERINFO({info:resin.data})
            if(!isNotBlank(getStore('mallId'))){
                removeStore('buyCart')
                setStore("mallId", resin.data.id);
            }else if(resin.data.id != getStore('mallId')){
                removeStore('buyCart')
                removeStore('mallId')
                setStore("mallId", resin.data.id);
            }
          ...
          // userInfo1("", { headers: { token: res.data } }).then(resin => {
          //   if (resin.success == 1) {
          //     setStore("userId", '62');
          //     if (this.cart.length) {
          ...
          // });
           ...
           this.$message.error({
              message:isNotBlank(res.msg)?res.msg:'登录失败'
           })
            this.logintxt = "登录";
        }
      });
...
.v2 .dialog {
  ...
  .title {
    ...
    position: relative;
    background-image: url(../../../static/images/mallLogo.jpg);
