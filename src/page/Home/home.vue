<template>
  <div class="home">
    <div v-loading="loading" element-loading-text="加载中..." style="min-height: 35vw;" v-if="!error">
        ...
        >
          <transition name="fade">
            <div
              ...
              @click="linkTo(item.categoryId)"
              @mouseover="stopTimer"
              @mouseout="startTimer"
            >
              <img v-if="item.path" class="img1" :src="showimg(item.path)" />
      ...
      <div v-for="(item) in indexPageGoods" :key="item.id">
      ...
      <section class="w mt30 clearfix" v-if="item.type === 2">
        ...
        <section class="w mt30 clearfix" v-if="item.goodsList&&item.goodsList.length>0">
          <y-shelf :title="item.name">
            <div slot="content" class="floors">
              <el-row style="width:100%">
                <mall-goods :msg="items" v-for="(items) in item.goodsList" :key="items.id"></mall-goods>
              </el-row>
            </div>
          </y-shelf>
        </section>
...
<script>
import { productHome, findIndexPhoto } from "/api/index.js";
import { getIndexPageGoods } from "/api/goods.js";
import YShelf from "/components/shelf";
import product from "/components/product";
import mallGoods from "/components/mallGoods";
import { setStore, getStore, getFullUrl, isNotBlank } from "/utils/storage.js";
export default {
  data() {
    return {
      error: false,
  ...
  mounted() {
    findIndexPhoto({ isSort: 0 }).then(res => {
      if (res.success != 1) {
        this.error = true;
        return;
      }
    ...
    getIndexPageGoods({}).then(response => {
      this.loading = false;
      if (
